...
export type NormalRandom = MPRandGauss;
...
function standardDeviation(values: TypedArray | number[], meanVal: number) {
    let squareDiffSum = 0;
    for (const v of values) {
        const diff = v - meanVal;
...
function testEpsilon(): number {
    return TEST_EPSILON_FLOAT32;
}

function expectNumbersClose(a: number, e: number, epsilon?: number) {
    if (epsilon == null) {
        epsilon = testEpsilon();
    }
...
function areClose(a: number, e: number, epsilon: number): boolean {
    ...
    return !(isNaN(a) || isNaN(e) || Math.abs(a - e) > epsilon);
}
